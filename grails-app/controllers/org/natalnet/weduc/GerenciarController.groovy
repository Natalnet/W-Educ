...
class GerenciarController {
	...
	def alternar() {
        ...
        def current = Privilegio.findWhere(authority: "ROLE_ALUNO")
        def up = UsuarioPrivilegio.get(aluno.id, current.id)

        up.privilegio = Privilegio.findWhere(authority: "ROLE_PROFESSOR")
            
        if(up.save(flush: true)){
            aluno.professor = null
            aluno.save(flush: true)
            render up.privilegio
            //redirect action: "usuarios" 
        }
//            
